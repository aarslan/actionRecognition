...
def create_empty_table(table_fname):
    ...
    class images(ta.IsDescription):
        ...
        features     = ta.UInt8(shape = (N_FEATURES_TOTAL))
    ...
    h5.close()
#------------------------------------------------------------------------------#
def read_mat_files(features_fname, labels_fname, names_fname):
    """docstring for read_mat_file"""

    print "reading features"
    tic = time.time()
    f = h5py.File(features_fname, 'r')
    ff = f["myData"]
...
def main():
    ...
    features, labels, aviNames = read_mat_files(features_fname, labels_fname, names_fname)
